...
%</Person>
%\end{verbatim}
%\caption{A person described in XML}
%\label{XMLexample}
%\end{figure}
...
\caption{A Starlink MDL specification\cite{Bromberg:2011}}
\label{MDL1}
\end{figure}
...
The Fuzzy Markup Language\cite{Acampora:2013} was not designed for interoperating systems at all. Fuzzy controllers handle converting crisp, or discrete, data into sets that cannot be defined as strictly. Fuzzy controlers may be used in things like automobile transmission control, where 
\mycomment{could perhaps takl about how FML is used to describe functionality, more than just data types. Else drop}
...
For issues like these, there are two concepts that help enable this translation with minimal loss: metadata and standard interfaces. \mycomment{I'm not sure if I should refer to standards as interfaces, or how to phrase that}
...
Although the above study found issues between virtual machines, systems run on homogeneous virtual machines architecture actually has an advantage. Systems on a virtual machine already have a standard implemented in the form of the intermediate language. Because the intermediate language has a full language specification, including both a type system and data interpretation behavior, it can act as a description for required behavior. Moreover, once a system has been compiled into the intermediate language, the behavior of the different components is constrained by that virtual machine's behavior.

This is not a perfect solution however, as it requires 

As Java was not initially intended to run other languages, making calls to non-Java languages is a difficult proposition\cite{Chisnall:2013}. However, new JVM languages, like Clojure and JRuby, inherrently provide support for accessing at least Java classes, if not directly other JVM languages.
...
Semantic interop is much easier to attain with markup languages. Although not built in, many markup languages have a notion of correct formatting. In XML, correct formatting can be determined with the use of schema and 

It should be noted that schema and similar mechanisms are themselves simply a standard, and are not expressly enforced or singular. XML has several 
...
In comparison, markup languages are better suited to dealing with preexisting or legacy systems, where there is too much code to change or parts of the source are simply unavailable,
